...
export default function NavbarComponent({ toggleTheme }) {
  ...
  return (
    <Navbar
      className="w-full top-0 mx-auto shadow-md dark:bg-neutral-950"
      classNames={{ wrapper: "container max-w-auto px-0 sm:px-6" }}
    >
      <NavbarBrand justify="start" className="gap-2">
        <Link className="flex items-center" to={"/"}>
          <img className="w-10 " src={logo} alt="Note Logo" />
          <h2 className="font-jost font-medium ms-2">
            Note
            <span className="bg-black text-white dark:bg-white dark:text-black px-1 rounded-xl">
              Plus
            </span>
          </h2>
        </Link>
